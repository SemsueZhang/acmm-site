...
有向无环图（Directed Acyclic Graph，DAG）一定至少有一个入度为 0 的点：否则从任意点不断沿入边向前走，有限个顶点中必然重复并形成环。这个性质既保证 Kahn 算法每一步都有合法候选，也给出检测环的依据。
...
正确性可按输出顺序归纳：点 $u$ 被取出时当前入度为 0，所有仍未输出顶点都没有边指向 $u$；已经删除的入边来源又都排在 $u$ 前，所以把 $u$ 放在当前位置不会违反任何边的先后关系。若算法卡住，剩余子图每点入度至少为 1，沿入边反复前进必然找到有向环。
...
方向映射是本题关键：一条食物链沿“食物到捕食者”的有向边前进。每条完整链有唯一终点，因此只在出度为 0 的点汇总不会重复；所有生产者的初值设为 1，保证每条链从唯一的入度 0 起点被计入。
