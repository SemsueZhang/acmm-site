...
- [洛谷 P7771【模板】欧拉路径](https://www.luogu.com.cn/problem/P7771)：先用入度、出度确定起点，再用 Hierholzer 后序记录路径；若要求字典序最小，需要按合适顺序取出邻接边。
- [洛谷 P1330 封锁阳光大学](https://www.luogu.com.cn/problem/P1330)：每条边两端必须分属不同阵营，先对每个连通分量二染色；若冲突则无解，否则选择两种颜色中人数较少的一侧。

两题都使用线性图遍历，但维护的不变量完全不同：欧拉路标记的是“边是否使用”，二分图标记的是“点的颜色”。不能因代码都有 DFS/BFS 就混用访问数组。
