...
本文默认序列元素互不相同，构造“小根、下标中序”的笛卡尔树。学完后应能解释树为何唯一、从朴素递归推导出单调栈构造，并独立处理左右孩子编号。重复值需要额外规定相等元素的先后关系，不属于本页主线。
...
识别线索是“按原下标保持中序，同时按权值满足堆序”。若逐个区间寻找最小值，单调排列会产生 $O(n^2)$；使用单调栈后，每个位置只进出栈一次。题目输出的异或式只是构造后的统计，不应混入树的维护逻辑。
...
构造时间 $O(n)$，统计时间 $O(n)$，左右孩子数组与栈共占 $O(n)$ 空间。可迁移的信号是：当题目同时要求保留下标顺序与区间最值层级时，应考虑笛卡尔树；单调栈维护的是尚可能成为后续元素祖先的最右链。
