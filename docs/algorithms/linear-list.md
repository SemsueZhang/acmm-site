...
[洛谷 P3613 寄包柜](https://www.luogu.com.cn/problem/P3613) 的柜子编号和格子编号范围很大，但实际使用位置较少。若直接创建完整二维数组会浪费大量空间；可以为每个柜子维护只记录已使用格子的关联容器。

这道题用于练习“先看访问方式再选容器”：需要按两个编号精确读写，不需要顺序遍历或连续内存，因此稀疏映射比链表或巨大二维 `vector` 更合适。容器不是按题面名词选择，而是按随机访问、插删和空间需求选择。
