...
本文统一令 `dp[j]` 表示处理完当前物品前缀后、总体积不超过 $j$ 的最大价值。所有体积均为正；若存在零体积正价值物品，完全背包可能无上界，需要另行判断。循环方向不是实现细节，而是“一件物品能否在同一层被再次使用”的数学含义。
...
转移按第 $i$ 件物品是否选择划分全部方案，两类互斥且完备。不选时保留前 $i-1$ 件的最优解；选择时删除该物品后，剩余方案只能来自容量 $j-w_i$ 的前缀子问题。两类分别取最优再取最大，因此满足最优子结构。
...
用反例检查方向：只有一件体积 2、价值 3 的物品，容量 4。0/1 背包若正序更新，会先得到 `dp[2]=3`，随后错误地用本轮 `dp[2]` 更新 `dp[4]=6`；倒序时 `dp[4]` 读取的是旧层 `dp[2]=0`，同一物品不会重复。
...
这里的建模是一一对应的：任意采药方案对应一个物品子集，总用时不超过容量；任意背包子集也对应可以执行的采药方案。题目只求最大价值且允许时间剩余，因此初值 0 表示“容量不超过”，不能换成“恰好装满”的负无穷初始化。
